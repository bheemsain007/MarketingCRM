...
WHATSAPP_PROVIDER, WHATSAPP_API_KEY, WHATSAPP_PHONE_NUMBER_ID, WHATSAPP_WEBHOOK_VERIFY_TOKEN, WHATSAPP_APP_SECRET, WHATSAPP_TEMPLATE_LANGUAGE
...
| Restore drill | ✅ **Mechanism built and verified 2026-09-06**, not merely policy: `crm:restore {file} --database=<target> --force` decrypts and restores a `crm:backup` file. A real drill was run against this project's own dev database — backup, restore into a scratch database, then every one of 57 tables' row counts compared and a `CHECKSUM TABLE` on `leads` matched bit-for-bit; `tests/Feature/Ops/BackupRestoreDrillTest.php` repeats this automatically on every test run (backup → restore into a throwaway database → compare row counts and a specific row's content → assert the `migrations` table itself survived, proving schema metadata restored, not just data). |
| **Cross-engine drill (T-02)** | ✅ **Also verified 2026-09-06.** This dev machine only runs MariaDB 10.4.32 day to day (T-48), while production is pinned to **MySQL 8.4.9** (T-02) — a backup taken on one engine restoring cleanly on the other was never actually checked until now. A MySQL 8.4.9 instance already installed on this machine (`C:\Program Files\MySQL\MySQL Server 8.4`, previously found competing for port 3306 — see the mysql.db repair history) was started on a scratch port, a fresh `crm:backup` of the MariaDB dev database was decrypted, and the resulting plain SQL was restored directly into MySQL 8.4 with no translation step. Result: all 57 tables matched row-for-row, sample rows matched byte-for-byte, and `SUM()`/`COUNT()` aggregates across four tables of varying column types matched exactly. **This is the strongest evidence available without an actual deployed host** that `crm:backup`'s output is portable to the real production engine, not just self-consistent on the engine that produced it. The MySQL 8.4 instance and scratch database used for this were stopped and dropped afterward — nothing from it persists |
| What a real production drill still needs | This proves the *mechanism* and now the *target engine* — it does not yet prove the actual production **host**: its own mysqldump/mysql binary versions, its disk I/O and storage location, and genuine off-site copy of the encrypted file. Re-run `crm:backup` / `crm:restore` for real once that host exists |
